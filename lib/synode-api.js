...
/**
 * Digs the public URL out of an upload response.
 *
 * `media/files/upload` does not answer with a `MessageDto` like the document
 * endpoints do — it returns the result of the GCS `setMetadata()` call
 * straight through, which is a bare `[metadata, apiResponse]` array. Accept the
 * wrapped shape too, in case that ever gets normalised.
 *
 * @param {object|Array} body Parsed response body
 * @returns {string|undefined}
 */
function mediaLinkOf(body)
{
	...
	for (const candidate of candidates)
	{
		...
		if (first?.mediaLink) return first.mediaLink;
	}
